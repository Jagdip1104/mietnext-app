    ...
    <div className="border border-gray-200 rounded-lg overflow-hidden shadow-sm">
      ...
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors
          ${isVacant ? 'bg-gray-50 hover:bg-gray-100' : 'bg-white hover:bg-gray-50'}`}
      >
        ...
        <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap">
          ...
          {unit.floor != null && (
            <span className="text-xs text-gray-400 flex items-center gap-0.5">
              <Layers className="h-3 w-3" />{unit.floor}. OG
            </span>
          )}
          {unit.area_sqm && (
            <span className="text-xs text-gray-400">{unit.area_sqm} m²</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {isVacant ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-500">
              <UserX className="h-3 w-3" />Leerstand
            </span>
          ) : (
            <>
        ...
        <div className="border-t border-gray-100 bg-gray-50 px-4 py-4">
          {isVacant ? (
            <div className="flex items-center justify-between">
              ...
              <Link
                href={`/tenants?unit=${unit.id}`}
                className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700 transition-colors"
              >
                <Plus className="h-3.5 w-3.5" />Mieter anlegen
              </Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide flex items-center gap-1">
                  <User className="h-3.5 w-3.5" />Mieter
                </p>
                <p className="text-sm font-medium text-gray-900">{unit.tenant!.name}</p>
                {unit.tenant!.email && (
                  <a href={`mailto:${unit.tenant!.email}`} className="flex items-center gap-1.5 text-xs text-blue-600 hover:underline">
                    <Mail className="h-3 w-3" />{unit.tenant!.email}
                  </a>
                )}
                {unit.tenant!.phone && (
                  <a href={`tel:${unit.tenant!.phone}`} className="flex items-center gap-1.5 text-xs text-gray-600">
                    <Phone className="h-3 w-3" />{unit.tenant!.phone}
                  </a>
                )}
                <div>
                  {status === 'active' && (
                    <span className="inline-flex items-center gap-1 text-xs text-emerald-700">
                      <CheckCircle2 className="h-3 w-3" />Portal-Zugang aktiv
                    </span>
                  )}
                  {status === 'invited' && (
                    <span className="inline-flex items-center gap-1 text-xs text-amber-700">
                      <Clock className="h-3 w-3" />Eingeladen {formatDate(unit.tenant!.invited_at)}
                    </span>
                  )}
                  {status === 'not_invited' && (
                    <Link href="/invite-tenant" className="inline-flex items-center gap-1 text-xs text-red-600 hover:underline">
                      <AlertCircle className="h-3 w-3" />Nicht eingeladen →
                    </Link>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide flex items-center gap-1">
                  <FileText className="h-3.5 w-3.5" />Vertrag
                </p>
                {unit.contract ? (
                  <>
                    <p className="text-sm font-semibold text-gray-900">
                      {formatCurrency(unit.contract.rent_amount)}
                      <span className="text-xs font-normal text-gray-500 ml-1">/ Monat</span>
                    </p>
                    <p className="text-xs text-gray-500">
                      {formatDate(unit.contract.start_date)} – {unit.contract.end_date ? formatDate(unit.contract.end_date) : 'unbefristet'}
                    </p>
                    <span className={`inline-block text-xs rounded-full px-2 py-0.5 font-medium
                      ${unit.contract.status === 'active'
                        ? 'bg-emerald-50 text-emerald-700 border border-emerald-100'
                        : 'bg-gray-100 text-gray-600'}`}>
                      {unit.contract.status === 'active' ? 'Aktiv' : unit.contract.status}
                    </span>
                  </>
                ) : (
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide flex items-center gap-1">
                  <CreditCard className="h-3.5 w-3.5" />Letzte Zahlung
                </p>
                {unit.lastPayment ? (
                  <>
                    ...
                    {unit.lastPayment.paid_date && (
                      <p className="text-xs text-gray-500">Bezahlt: {formatDate(unit.lastPayment.paid_date)}</p>
                    )}
                    <PaymentBadge payment={unit.lastPayment} />
                  </>
                ) : (
...
export default function PropertyDetailPage() {
  ...
  async function loadData() {
    ...
    const { data: prop, error: propErr } = await supabase
      .from('properties')
      .select('id, name, address, city, zip, type')
      .eq('id', propertyId)
      .eq('owner_id', user.id)
      .single()

    if (propErr || !prop) {
      setError('Objekt nicht gefunden oder kein Zugriff.')
      setLoading(false)
      return
    }
    setProperty(prop)
    ...
    const { data: rawUnits } = await supabase
      .from('units')
      .select('id, name, unit_code, floor, area_sqm')
    ...
    const { data: tenants } = await supabase
      .from('tenants')
      .select('id, name, email, phone, user_id, invited_at, unit_id')
      .in('unit_id', unitIds)

    // 4. Contracts (latest active per unit)
    ...
    // 5. Payments (latest per contract)
    ...
    for (const c of contracts ?? []) {
      if (!contractByUnit.has(c.unit_id)) contractByUnit.set(c.unit_id, c)
    }

    const lastPaymentByContract = new Map<string, Payment>()
    for (const p of payments ?? []) {
      if (!lastPaymentByContract.has(p.contract_id)) lastPaymentByContract.set(p.contract_id, p)
    }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      {/* Back + Header */}
      <div>
        <Link href="/properties" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 mb-4">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <Link
            href={`/units?property=${propertyId}`}
            className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors flex-shrink-0"
          >
      ...
      {units.length > 0 && (
        <>
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setExpandAll(v => !v)} className="text-xs text-blue-600 hover:underline">
      ...
      {units.length === 0 ? (
        ...
        <div className="space-y-2">
          {units.map(unit => (
            <ExpandableUnit key={unit.id} unit={unit} forceOpen={expandAll} />
          ))}
        </div>
      )}
    </div>
